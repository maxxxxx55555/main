...
def welcome(name: str, catalog: PlanCatalog) -> str:
    ...
    pro = catalog.get("pro")
    return (
        f"👋 Привет, {escape_html(name)}!\n\n"
        "Я — <b>AI-Сотрудник</b>: отвечаю вашим клиентам 24/7, квалифицирую лидов, "
        "помогаю с записью на консультации и не пропускать обращения.\n\n"
        "Просто напишите мне вопрос — как будто это тестовый клиент.\n"
        "А чтобы я отвечал от лица вашего бизнеса, загрузите базу знаний "
        "(FAQ, цены, условия) в разделе «📚 База знаний».\n\n"
        f"💡 <b>{free.message_limit} сообщений в месяц — бесплатно</b>, без карты. "
        f"На PRO (⭐{pro.price_stars}) — {pro.message_limit} сообщений + база знаний: "
        "я отвечаю строго как сотрудник именно вашего бизнеса.\n\n"
...
def help_text(settings: Settings) -> str:
    ...
    return (
        "<b>Что я умею</b>\n"
        "• Отвечаю клиентам на вопросы (по вашей базе знаний на PRO/Business)\n"
        "• Квалифицирую лидов: имя, потребность, срочность\n"
        "• Помогаю записать клиента на консультацию\n\n"
        "<b>Команды</b>\n"
        "/start — начать работу\n"
        "/help — эта справка\n"
        ...
        "💬 Чтобы проверить бота — просто напишите вопрос текстом.\n"
        "📄 Базу знаний можно загрузить текстом или .txt/.md файлом."
