...
| **Б: Render.com Free (Web Service + пингер)** | средняя | 30-60 сек без пингера; мгновенно с пингером | демонстрация, лёгкий прод |
...
### Б2. Создание сервиса
1. https://render.com → Sign Up (через GitHub) → **New +** → **Web Service**
   (Background Worker на Free-плане недоступен; Web Service бесплатен, но засыпает — см. Б3)
...
3. **Environment** → добавить переменные:
   ...
   LOG_LEVEL        = INFO
   ```
   Порт Render передаёт в `PORT` — приложение читает его автоматически
   (`WEBAPP_PORT` тоже поддерживается), ничего вручную задавать не нужно.
4. **Create Resource** → деплой 3-5 минут → в логах: «Бот запущен (long polling)».
   Миграции применяются автоматически при старте.

### Б2.5. Не даём сервису засыпать (бесплатно)
Render Free останавливает Web Service после 15 минут **без входящего HTTP-трафика**.
Polling в Telegram — это исходящие запросы, их Render не видит, поэтому добавьте
бесплатный внешний пингер:
1. UptimeRobot (или cron-job.org) → новый HTTP-монитор на
   `https://<ваш-сервис>.onrender.com/health` с интервалом **5 минут**
2. Это держит сервис активным 24/7 в пределах бесплатных лимитов.

> Без пингера первый ответ после простоя займёт 30-60 секунд (холодный старт).

### Б3. Честные ограничения Free Tier

| Ограничение | Что значит | Митигация |
|---|---|---|
| **Sleep после 15 мин без HTTP-трафика** | холодный старт 30-60 сек на первый ответ | бесплатный пингер `/health` каждые 5 минут (см. Б2.5) |
